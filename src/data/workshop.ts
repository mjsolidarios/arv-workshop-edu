    ...
    samplePrompts: [
      {
        ...
        prompt:
          "You are helping me plan a higher-education lesson in Gemini Notebook. Discipline: Environmental Science. Course/year level: first-year college. Course learning outcome: analyze the movement of water through an ecosystem and explain how human activity affects it. Produce a complete lesson plan that integrates either (a) smartphone image-tracked AR with Encantar.js (browser image tracking; not QR codes or markerless WebXR) or (b) hand-gesture interaction with Google MediaPipe Tasks Vision Hand Landmarker (landmarks mapped to answers in the app). Include: lesson title, prior knowledge, 20-minute procedure, authentic task, chosen camera interaction and why it strengthens the outcome, materials and device setup, game elements (points, feedback, progress), evidence of learning, accessibility alternative when the camera is unavailable, privacy notes (camera frames stay on-device), and a short content outline for later app development. In the handoff notes, name the build stack: Gemini Notebook → Google Stitch → Google AI Studio (or Lovable/Bolt/Replit backup) plus Encantar.js with Three.js or PixiJS for AR overlays, or MediaPipe for gesture/pose. Also recommend which Studio outputs to generate next (slide deck of the procedure, video overview of the learner journey, or infographic of the water-cycle stations) so I can better visualize and refine the design. Keep classroom setup under 15 minutes.",
      },
      {
        title: "Lesson idea generator",
        useFor: "Brainstorming a first activity idea",
        prompt:
          "I teach a first-year Environmental Science course at a Philippine college. Course learning outcome: analyze the movement of water through an ecosystem and explain how human activity affects it. Suggest 3 short gamified activities: at least one using Encantar.js image-tracked AR on a phone browser, and at least one using Google MediaPipe hand or pose landmarks on a laptop webcam. For each idea, list the outcome alignment, materials, rules, scoring, evidence of learning, which library powers the camera interaction (Encantar.js or MediaPipe), and how AI can help prepare content for a later Google AI Studio build. Keep setup under 15 minutes.",
      },
      {
        title: "Quiz questions at the right level",
        useFor: "Generating review items for a MediaPipe gesture quiz",
        prompt:
          "Create 8 concept-check questions for an introductory Statistics course on selecting and interpreting measures of central tendency, formatted for a browser gesture quiz built with Google MediaPipe Hand Landmarker (thumbs-up = choice A, open palm = choice B, or A/B zones). Include 2 foundational, 4 application, and 2 analysis items. For each item provide: question, two plausible interpretations labeled A and B, correct answer, and a concise rationale. Use authentic Philippine higher-education or community data contexts and avoid trick wording. Flag any item that needs faculty verification before class use.",
      },
    ],
    workedExamples: [
      {
    ...
    topics: [
      {
        ...
        points: [
          ...
          "Step 7: Test on a laptop and a smartphone — camera permission, lighting, distance, and orientation matter.",
          "Step 8: Refine the app for clarity, fairness, and learning effectiveness (not just “cool factor”).",
        ],
      },
      {
        heading: "Lesson Planning with Gemini Notebook (AR & Motion)",
        points: [
          "Start in Gemini Notebook with the syllabus outcome, topic sources, and class constraints (devices, class size, time).",
      ...
      },
      {
        ...
        points: [
          "Describe the learning objective, target learners, camera interaction, required screens, and the feeling the interface should create—using the Gemini Notebook plan as the source of truth.",
    ...
    samplePrompts: [
      {
        ...
        prompt:
          "Create a complete higher-education lesson plan in Gemini Notebook for a first-year Botany laboratory. Course learning outcome: identify major plant structures from specimen images and relate visible structure to function. Integrate smartphone image-tracked AR powered by Encantar.js (browser image tracking of instructor-supplied specimen photos; not QR codes or markerless WebXR) as the core interaction, with Three.js or PixiJS for overlay cards. Include: lesson title, time (20 minutes), prior knowledge, authentic task, why AR strengthens this outcome, materials, device setup, game elements (progress, score, feedback), step-by-step teacher procedure, evidence of learning, accessibility alternative without the camera, privacy notes (on-device frames only), and a content outline for four plant structures (name, function, one analysis question, brief feedback). End with design notes ready to hand to Google Stitch and Google AI Studio that name Encantar.js, required reference-image assets, and overlay library choice, and recommend Studio outputs (slide deck of the lab flow, video overview of the AR stations, or infographic of the four structures and scoring) to help visualize and refine the plan.",
      },
      {
        title: "Design the app in Google Stitch first",
        useFor: "Planning the interface and learner flow before development",
        prompt:
          "Design a mobile-first laboratory review app called ‘Scan-to-Analyze Plant Anatomy’ for first-year Botany students. The course outcome is to identify major plant structures from specimen images and relate visible structure to function. The finished app will use Encantar.js for image tracking and Three.js or PixiJS for AR overlays, built later in Google AI Studio. Create and connect these screens: (1) welcome and Start button, (2) concise camera-permission and laboratory-use instructions, (3) live AR activity with a large camera area, Encantar tracking status (searching / tracked / lost), progress 0/4, score, and a visible non-camera fallback, (4) an analysis question with large answer controls and evidence-based feedback, and (5) results plus a two-item reflection. Use accessible contrast, large touch targets, and a clean laboratory visual style. Design for phone portrait first and include a laptop layout. The interface must not imply that camera frames are recorded or uploaded. Create a clickable prototype, then summarize the user flow, reusable design rules, components, and Encantar.js implementation notes for Google AI Studio development.",
      },
      {
        title: "Build an image-tracked AR learning app",
        useFor: "Creating a complete Encantar.js AR app in Google AI Studio",
        prompt:
          "Build a mobile-first browser app for a first-year Botany laboratory activity titled ‘Scan-to-Analyze Plant Anatomy’ in Google AI Studio (or Lovable, Bolt, or Replit if tokens run out). Required stack: Encantar.js for image tracking and Three.js or PixiJS for the AR overlay—do not substitute QR scanning, barcode markers, AR.js, 8th Wall, or markerless WebXR. The app must register and recognize four instructor-supplied specimen or diagram images as Encantar image targets. When a target is tracked (targetfound), anchor a readable overlay card with the structure name, functional significance, and one application or analysis question; clear or hide the overlay on targetlost. Award points using a transparent rubric and show progress out of 4. Include a start screen, concise camera instructions, permission handling, loading and tracking status, reset, and a non-camera fallback containing the same evidence. Keep all Encantar registration and camera processing in the browser; do not upload or record frames. Use accessible controls and terminology appropriate for an introductory college course. Generate all app files and sample content, clearly mark where the four reference images belong, load Encantar from a documented CDN or local vendor path, and flag scientific claims for faculty verification. Explain how to test the app on a phone over HTTPS or localhost.",
      },
      {
        title: "Build a vision-controlled gesture quiz",
        useFor: "Creating a complete MediaPipe app in Google AI Studio",
        prompt:
          "Build a responsive browser app called ‘Statistics Claim Check’ for an introductory college Statistics course in Google AI Studio (or Lovable, Bolt, or Replit if tokens run out). Required stack: Google MediaPipe Tasks Vision Hand Landmarker running in the browser (VIDEO mode, one hand)—do not replace MediaPipe with TensorFlow.js pose models, raw OpenCV, or cloud vision APIs. Map a clearly detected thumbs-up to SUPPORTED and an open palm to NOT SUPPORTED using landmark-based gesture rules or MediaPipe Gesture Recognizer when available. Show a mirrored camera preview, detected gesture and confidence, and 10 evidence-based claims about interpreting graphs, averages, correlation, and sampling. Require a gesture to remain stable for about 800 milliseconds, then lock input until the next claim. Award 1 point per defensible classification and show a concise rationale, progress, final score, and restart. Include camera permission/errors, keyboard and touch equivalents, and an untimed option. Process video locally with MediaPipe; never save or upload frames. Use accessible, high-contrast controls. Generate all app files, load MediaPipe via the official Tasks Vision package or CDN, faculty-verifiable claims, and testing instructions.",
      },
    ],
    workedExamples: [
      {
    ...
    samplePrompts: [
      {
        ...
        prompt:
          "Plan a complete 20-minute higher-education lesson in Gemini Notebook. Discipline: [discipline]. Course/year level: [level]. Course learning outcome: [paste outcome]. Camera interaction: [image-tracked AR with Encantar.js OR hand gestures with MediaPipe Hand Landmarker OR body pose with MediaPipe Pose Landmarker]. Produce: lesson title, prior knowledge, authentic task, why the chosen interaction strengthens the outcome, materials and device setup, game rules and scoring, step-by-step procedure, evidence of learning, accessibility alternative, privacy notes (on-device camera processing), content outline (items or markers), and handoff notes for Google Stitch (screens) and Google AI Studio (build stack). In the handoff, name the exact libraries: Encantar.js plus Three.js or PixiJS for AR overlays, or Google MediaPipe Tasks Vision for gesture/pose—do not plan QR-only or cloud vision substitutes. After the plan, suggest which Studio outputs to generate next (slide deck of the learner flow, video overview of the activity, or infographic of rules/stations) to help visualize and refine the design. Keep it feasible with 1 laptop and 1 smartphone.",
      },
      {
        title: "Align the activity to a course outcome",
        useFor: "Design planning",
        prompt:
          "Course learning outcome: [paste outcome]. Course/year level: [level]. Discipline: [discipline]. Design a 20-minute higher-education activity using either Encantar.js image-tracked AR on a smartphone browser or Google MediaPipe hand/pose landmarks on a laptop webcam. Include: outcome alignment, authentic task, materials, procedure, transparent scoring criteria, evidence of learning, accessibility alternative, the specific library and interaction rules (e.g., which Encantar targets or MediaPipe landmarks/gestures), and a 3-item formative check. Keep it feasible with 1 laptop and 1 smartphone.",
      },
      {
        title: "Build an Encantar.js AR classroom app",
        useFor: "First working AR build in Google AI Studio",
        prompt:
          "Build a mobile-friendly browser learning app in Google AI Studio (or Lovable, Bolt, or Replit if tokens run out). Discipline: [discipline]. Course/year level: [level]. Course learning outcome: [paste outcome]. App title: [title]. Required stack: Encantar.js for image tracking and Three.js or PixiJS for overlay content. Do not use QR scanning, AR.js, or markerless WebXR instead of Encantar.js. Register [number] instructor-supplied reference images. On targetfound, show an anchored card with faculty-verifiable content and one analysis question; handle targetlost, permissions, loading, progress, scoring, reset, and a non-camera fallback. Keep frames on-device. Generate complete app files, mark asset paths for the reference images, and give phone HTTPS testing steps.",
      },
      {
        title: "Build a MediaPipe gesture or pose quiz",
        useFor: "First working vision-controlled build in Google AI Studio",
        prompt:
          "Build a responsive browser learning app in Google AI Studio (or Lovable, Bolt, or Replit if tokens run out). Discipline: [discipline]. Course/year level: [level]. Course learning outcome: [paste outcome]. App title: [title]. Required stack: Google MediaPipe Tasks Vision—use Hand Landmarker for gestures or Pose Landmarker for body poses (choose one). Do not replace MediaPipe with cloud vision APIs. Map clear, debounced landmark rules to answers (e.g., thumbs-up vs open palm, or named pose matches). Include mirrored preview, confidence or status, [number] faculty-verifiable items, scoring, feedback rationales, keyboard/touch fallback, and local-only camera processing. Generate complete app files and a device-testing checklist.",
      },
      {
    ...
    topics: [
      ...
      },
      {
        ...
        points: [
          ...
          "Uses generative AI for at least one component (lesson plan, content, images, instructions, or questions).",
    ...
    samplePrompts: [
      {
        ...
        prompt:
          "Create a complete classroom lesson plan in Gemini Notebook. Discipline: [DISCIPLINE]. Course/year level: [LEVEL]. Course learning outcome: [PASTE OUTCOME]. App or activity title: [TITLE]. Camera interaction and library: [IMAGE-TRACKED AR with Encantar.js + Three.js or PixiJS / HAND GESTURE with MediaPipe Hand Landmarker / BODY POSE with MediaPipe Pose Landmarker]. Learner action: [DESCRIBE]. Game elements: [POINTS / LEVELS / TIMER / BADGES]. Produce: prior knowledge, authentic task, why this camera interaction strengthens the outcome, materials, device setup, privacy and accessibility notes (on-device frames only), 15–20 minute procedure, scoring and feedback, evidence of learning, content outline for challenges or markers, and handoff notes for Google Stitch (screens and states) and Google AI Studio (exact libraries, assets, and build requirements—no QR-only or cloud-vision substitutes). Recommend Studio outputs to generate next (slide deck of the procedure and screens, video overview of the learner journey, infographic of rules or stations, or mind map of outcomes to tasks) so the team can better visualize and refine the design. Keep it feasible with 1 laptop and 1 smartphone.",
      },
      {
        title: "Plan my classroom app in Google Stitch",
        useFor: "Customizable design prompt before the workshop build",
        prompt:
          "Design a mobile-first learning app. Discipline: [DISCIPLINE]. Course/year level: [LEVEL]. Course learning outcome: [PASTE OUTCOME]. App title: [TITLE]. Camera interaction and library: [IMAGE-TRACKED AR with Encantar.js / HAND GESTURE with MediaPipe Hand Landmarker / BODY POSE with MediaPipe Pose Landmarker]. Learner action: [DESCRIBE]. Game elements: [POINTS / LEVELS / TIMER / BADGES]. Create a connected prototype with welcome, concise consent and camera instructions, authentic course task, library-specific loading/error/tracking states (e.g., Encantar searching/tracked/lost or MediaPipe model loading/running), evidence-based feedback, progress, results, reflection, and a non-camera fallback. Use accessible contrast, readable type, large controls, and phone and laptop layouts. Summarize the final user flow, component list, design rules, interaction states, and Google AI Studio implementation notes that name Encantar.js or MediaPipe and any overlay library (Three.js or PixiJS for AR).",
      },
      {
        title: "Build my classroom camera app",
        useFor: "Customizable first-build prompt for the workshop",
        prompt:
          "Using the attached or pasted Google Stitch design, build a complete mobile-friendly browser learning app in Google AI Studio (or Lovable, Bolt, or Replit if tokens run out). Preserve its approved flow, component hierarchy, visual design, accessibility rules, and fallback states. Discipline: [DISCIPLINE]. Course/year level: [LEVEL]. Course learning outcome: [PASTE OUTCOME]. App title: [TITLE]. Camera interaction—pick one required stack and implement it fully: (A) IMAGE-TRACKED AR: Encantar.js image tracker + Three.js or PixiJS overlays; register instructor reference images; use targetfound/targetlost; do not substitute QR scanning, AR.js, or markerless WebXR; or (B) HAND GESTURE: Google MediaPipe Tasks Vision Hand Landmarker (VIDEO mode) with debounced landmark or Gesture Recognizer rules; or (C) BODY POSE: Google MediaPipe Pose Landmarker with calibrated joint-angle or landmark-match rules. Learner action: [DESCRIBE THE TARGET, GESTURE, OR POSE]. Create [NUMBER] authentic challenges with faculty-verifiable answers, transparent scoring, and feedback that explains disciplinary reasoning. Implement camera permission and loading states, live interaction status, progress, restart, and a final reflection or application task. Add touch or keyboard controls that provide equivalent participation when camera use is unavailable. Keep camera frames on the device and do not record or upload them. Generate the working app files, import Encantar.js or MediaPipe from a documented CDN/package, comment the camera-interaction logic, identify required assets, and provide a device-testing checklist. If a Stitch element cannot be implemented as designed, explain the limitation and retain the closest accessible behavior. Do not claim a gesture, pose, or image target works until it has been tested.",
      },
      {
        title: "Fix and refine the generated app",
        useFor: "Iterating after the first camera test",
        prompt:
          "Review the current app and fix only the problems listed below without removing working features or swapping libraries. Keep Encantar.js for image tracking or Google MediaPipe for hand/pose landmarks as already chosen—do not replace them with QR codes, cloud vision, or unrelated frameworks. Problems observed: [PASTE ERRORS OR DESCRIBE WHAT HAPPENED]. Target device and browser: [DEVICE + BROWSER]. Expected behavior: [DESCRIBE]. First explain the likely cause briefly. Then update the app so it: (1) shows useful camera, model-loading, and tracking errors, (2) avoids counting the same gesture, pose, or target more than once (debounce / cooldown), (3) remains readable on a phone in portrait orientation, (4) keeps touch/keyboard fallback controls working, and (5) still processes camera frames only on-device. After editing, list the exact checks I should perform to confirm the fix, including one Encantar targetfound or MediaPipe landmark path.",
      },
    ],
    workedExamples: [
      {
...
export const classroomExamples: ClassroomExample[] = [
  {
    ...
    aiPrompts: [
      ...
      "Build a responsive browser gesture quiz in Google AI Studio using Google MediaPipe Tasks Vision Hand Landmarker (not cloud vision). Map thumbs-up to interpretation A and open palm to interpretation B with ~800 ms debounce. Include mirrored preview, score, progress, keyboard fallback, and local-only camera processing. Load these 12 faculty-reviewed items: [paste items]. Generate complete app files and a short testing checklist.",
    ],
  ...
  },
  {
    ...
    aiPrompts: [
      ...
      "Build a mobile-first browser AR lab app in Google AI Studio with Encantar.js image tracking and Three.js or PixiJS overlays (not QR codes or markerless WebXR). Register six instructor specimen images as Encantar targets. On targetfound, show structure name, diagnostic feature, and analysis question; handle targetlost, permissions, progress, scoring, and a non-camera fallback. Keep frames on-device. Use this station content: [paste]. Generate complete files and phone HTTPS test steps.",
    ],
  ...
  },
  {
    ...
    aiPrompts: [
      ...
      "Build a laptop browser pose activity in Google AI Studio using Google MediaPipe Pose Landmarker (not cloud vision). For each of five hydrologic processes, detect a simple calibrated pose from landmarks, show status and scenario text, and require a short written explanation before awarding the reasoning point. Include seated alternatives, permission handling, and local-only camera processing. Use this pose content: [paste]. Generate complete app files and a safety testing checklist.",
    ],
  ...
  },
  {
    ...
    aiPrompts: [
      ...
      "Build a browser Statistics claim arena in Google AI Studio with Google MediaPipe Hand Landmarker. Map a stable hand position in a left screen zone to claim A and a right zone to claim B (debounced). Show mirrored preview, levels, scoring, statistical rationales, and keyboard fallback. Process frames only in the browser with MediaPipe. Use these items: [paste]. Generate complete app files and a testing checklist.",
    ],
  ...
  },
  {
    ...
    aiPrompts: [
      ...
      "Build a mobile browser local-history AR app in Google AI Studio using Encantar.js image tracking and Three.js or PixiJS overlays (not QR codes or WebXR). Register five map-style reference images. On targetfound, show a sourced exhibit card with narrative, citation, and discussion question; never invent historical facts. Include permissions, tracking status, checklist progress, and a non-camera packet fallback. Content: [paste faculty-verified labels]. Generate complete files and phone test steps.",
    ],
  ...
  },
  {
    ...
    aiPrompts: [
      ...
      "Build a browser claim–evidence–reasoning sorter in Google AI Studio with Google MediaPipe Hand Landmarker. Use three clearly distinct, debounced gestures (or two gestures plus a keyboard key for the third category). Show excerpt, live gesture status, feedback with textual cues, streak, and local-only camera processing. Items: [paste]. Generate complete app files and accessibility notes.",
    ],
  ...
  },
  {
    ...
    aiPrompts: [
      ...
      "Build a laptop browser form-check coach in Google AI Studio using Google MediaPipe Pose Landmarker only for coarse participation feedback—not clinical assessment. For each station, show demo cues, detect a simple pose match with a timer, award attempt/improvement points, and always show safer modifications. Keep teacher override and local-only camera processing. Station content: [paste]. Generate complete app files and a safety disclaimer checklist.",
    ],
  ...
  },
  {
    ...
    aiPrompts: [
      ...
      "Build a mobile browser mini-gallery AR app in Google AI Studio with Encantar.js image tracking and Three.js or PixiJS overlays (not QR codes). Allow registration of student artwork photos as Encantar reference images (or a fixed set of tested targets). On targetfound, show title, artist statement, process note, and a describe–analyze–interpret critique prompt. Include tracking status, non-camera fallback labels, and on-device processing only. Sample statements: [paste]. Generate complete files and a pre-gallery target-testing checklist.",
    ],
